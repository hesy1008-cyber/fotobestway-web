...
import { translateProductList } from "@/app/lib/translate";
...
export default async function ProductsPage({
  ...
  const currentSubCategory = search.subCategory || "";
  ...
  if (locale === "zh") {
    displayProducts = await translateProductList(products);
  }
            ...
            return activeCategory && activeCategory.subCategories.length > 0 ? (
              <div className="subCategoryMenu">
                {activeCategory.subCategories.map((subCat, subIndex) => (
                  <Link
                    key={subCat.slug}
                    href={`/${locale}/products?category=${activeCategory.slug}&subCategory=${subCat.slug}`}
        ...
        <section className="productListSection">
          ...
          >
            {displayProducts.length === 0 ? (
              ...
              displayProducts.map((product) => (
                <ProductListCard key={product.id} product={product} locale={locale} />
              ))
            )}
          </section>
